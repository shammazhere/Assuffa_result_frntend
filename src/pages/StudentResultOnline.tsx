...
const StudentResultOnline: React.FC = () => {
    ...
    const maxMarks = (typedStudent.marks?.length ?? 0) * 50; // Changed from 100 to 50
    ...
    return (
        <div style={{
            minHeight: '100vh',
            backgroundColor: '#FAFAF9', // Solid fallback
                    ...
                    <div style={{ padding: 'clamp(1rem, 4vw, 2rem)', background: '#FAFAF9', borderBottom: '1px solid #E7E5E4' }}>
                        <div style={{
                            ...
                            {[
                                ...
                                { label: 'Course Enrolled', value: `${(typedStudent.class?.name || 'N/A').toUpperCase()} - ${(typedStudent.class?.type || 'Online').toUpperCase()}` },
                            ].map(item => (
